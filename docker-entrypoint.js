...
if (STORAGE_TYPE === 'amazon-s3') {
	if (!STORAGE_S3_ACCESS_KEY_ID || !STORAGE_S3_SECRET_ACCESS_KEY || !STORAGE_S3_BUCKET || !STORAGE_S3_REGION) {
		throw new Error('Missing required environment variables for Amazon S3 storage');
	}

	config.storage.bucket = STORAGE_S3_BUCKET;
	config.storage.clientOptions = {
	...
	if (STORAGE_S3_ENDPOINT) {
		config.storage.clientOptions.endpoint = STORAGE_S3_ENDPOINT;
	}
